package com.hhd.idiom.pinyin4j;
...
/**
 * Created by huhengda on 2021/3/17.
 */
public class Pinyin4jWithToneNumber {
    private ArrayListMultimap<String,String> duoYinZiMap;
    public Pinyin4jWithToneNumber(){
        Dictionary.getDefault().init();
        duoYinZiMap = Dictionary.getDefault().getDuoYinZiMap();
    }
    ...
    public String getPinyin(String chinese) {
        ...
        for(int i=0;i<chs.length;i++){
            ...
            if(py_arr==null || py_arr.length<1){
                throw new RuntimeException("pinyin array is empty, char:"+chs[i]+",chinese:"+chinese);
            }
            if(py_arr.length==1){
